...
-- DROP FUNCTION IF EXISTS ce_etl.fx_tb_api_ud_dx;

CREATE OR REPLACE FUNCTION ce_etl.fx_tb_api_ud_dx(
...
DECLARE
    ...
    _AC_TYPE    INT := 1;
    _FILENAME   TEXT := 'ce_etl.fx_tb_api_ud_dx';
...
BEGIN
    ...
    FROM ce_etl.c_api_calc calc
    JOIN ce_etl.c_series src
      ON src.s_series_id = calc.ca_source_series
     AND src.s_active
     AND src.error IS NULL
    JOIN ce_etl.c_series tgt
...
COMMENT ON FUNCTION ce_etl.fx_tb_api_ud_dx
