 ...
 ***********************************************************************************************************
 * @file
 * fx_val__is_const_expr.sql
 *
 * Validation function - check constant expression.
 ***********************************************************************************************************
 */

-- DROP FUNCTION IF EXISTS ce_warehouse.fx_val__is_const_expr;

CREATE OR REPLACE FUNCTION ce_warehouse.fx_val__is_const_expr(
    _expr TEXT,
...
BEGIN
    ...
    IF _m IS NOT NULL THEN
        ...
        ELSEIF NOT EXISTS (SELECT 1 FROM ce_warehouse.l__pdi p WHERE p.pk_pdi = _pdi1::INT) THEN
    ...
    IF _m IS NOT NULL THEN
        ...
        ELSEIF NOT EXISTS (SELECT 1 FROM ce_warehouse.l__pdi p WHERE p.pk_pdi = _pdi1::INT) THEN
            RETURN FORMAT('Period "%s" does not exist', _pdi1);
        ELSEIF NOT EXISTS (SELECT 1 FROM ce_warehouse.l__pdi p WHERE p.pk_pd2 = _pdi2::INT) THEN
...
COMMENT ON FUNCTION ce_warehouse.fx_val__is_const_expr
