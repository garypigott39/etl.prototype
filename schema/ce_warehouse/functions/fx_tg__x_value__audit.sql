 ...
 ***********************************************************************************************************
 * @file
 * fx_tg__x_value__audit.sql
 *
 * Trigger function - log changes in x_value to custom audit table (AFTER update).
 ***********************************************************************************************************
 */

-- DROP FUNCTION IF EXISTS ce_warehouse.fx_tg__x_value__audit;

CREATE OR REPLACE FUNCTION ce_warehouse.fx_tg__x_value__audit(
)
    RETURNS TRIGGER
    LANGUAGE plpgsql
AS
...
BEGIN
    ...
    _sid1 := (SELECT sid1 FROM ce_warehouse.c_series WHERE pk_series = COALESCE(NEW.fk_pk_series, OLD.fk_pk_series));
    ...
    IF TG_OP = 'INSERT' THEN
        ...
        INSERT INTO ce_warehouse.x_series_meta (
            fk_pk_series, ifreq, itype, sid1, first_pdi, last_pdi, has_values, new_values_utc
        )
        VALUES (
    ...
    ELSEIF TG_OP = 'UPDATE' THEN
        ...
        IF (OLD.value IS DISTINCT FROM NEW.value)
            ...
            INSERT INTO ce_warehouse.x_series_meta (
                fk_pk_series, ifreq, itype, sid1, has_values, updated_values_utc
            )
            VALUES (
    ...
    ELSEIF TG_OP = 'DELETE' THEN
        ...
        INSERT INTO ce_warehouse.x_series_meta(fk_pk_series, ifreq, itype, first_pdi, last_pdi, has_values, updated_values_utc)
        VALUES (
...
COMMENT ON FUNCTION ce_warehouse.fx_tg__x_value__audit
