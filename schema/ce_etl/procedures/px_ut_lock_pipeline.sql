...
BEGIN
    ...
    ELSEIF _type = 'lock' THEN
        IF (SELECT COUNT(*) FROM ce_etl.s_pipeline_lock WHERE name = _name AND locked_utc > _ts) > 0 THEN
            RAISE EXCEPTION 'Pipeline "%" is already locked', _name;
        ELSE
            DELETE FROM ce_etl.s_pipeline_lock WHERE name = _name;
            INSERT INTO ce_etl.s_pipeline_lock(name) VALUES (_name);
