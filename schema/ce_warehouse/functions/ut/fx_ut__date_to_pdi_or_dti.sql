 ...
 ***********************************************************************************************************
 * @file
 * fx_ut__date_to_pdi_or_dti.sql
 *
 * Utility function - convert date/freq to period INT (pseudo primary key) or date INT (if frequency -1).
 ***********************************************************************************************************
 */

-- DROP FUNCTION IF EXISTS ce_warehouse.fx_ut__date_to_pdi_or_dti;

CREATE OR REPLACE FUNCTION ce_warehouse.fx_ut__date_to_pdi_or_dti(
    _date DATE,
    _ifreq INT
)
    RETURNS INT
...
COMMENT ON FUNCTION ce_warehouse.fx_ut__date_to_pdi_or_dti
