...
-- DROP FUNCTION IF EXISTS ce_warehouse.fx_tb_seq_check;

CREATE OR REPLACE FUNCTION ce_warehouse.fx_tb_seq_check(
)
    RETURNS TABLE (
        table_name TEXT,
        col_name TEXT,
...
BEGIN
    ...
    DROP TABLE IF EXISTS t__fx_tb_seq_check;
    CREATE TEMP TABLE t__fx_tb_seq_check
    (
        schema_name TEXT,
        table_name TEXT,
        col_name TEXT,
    ...
    INSERT INTO t__fx_tb_seq_check(schema_name, table_name, col_name, seq_name)
        SELECT
    ...
    FOR _rec IN SELECT * FROM t__fx_tb_seq_check
    LOOP
        ...
        UPDATE t__fx_tb_seq_check SET (seq_last_value, col_max_value) = (_lv, _mx) WHERE idx = _rec.idx;
    END LOOP;
    ...
    RETURN QUERY(
        SELECT
            ...
            CASE
                ...
                ELSE NULL
        END
        FROM t__fx_tb_seq_check t
        ORDER BY 1, 2
    );
END
$$;

COMMENT ON FUNCTION ce_warehouse.fx_tb_seq_check
