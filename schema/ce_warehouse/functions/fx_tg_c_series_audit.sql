...
-- DROP FUNCTION IF EXISTS ce_warehouse.fx_tg_c_series_audit;

CREATE OR REPLACE FUNCTION ce_warehouse.fx_tg_c_series_audit(
)
    RETURNS TRIGGER
    LANGUAGE plpgsql
AS
...
BEGIN
    ...
    ELSEIF TG_OP = 'UPDATE' THEN
        IF OLD.s_series_id IS DISTINCT FROM NEW.s_series_id THEN
            INSERT INTO ce_warehouse.a_cseries (pk_s, s_series_id, audit_type)
...
COMMENT ON FUNCTION ce_warehouse.fx_tg_c_series_audit
