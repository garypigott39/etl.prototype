...
WITH _periods AS (
    -- DAILY
    SELECT
        ce_warehouse.fx_ut_date_to_pdi(d_date, 1) AS pk_p,
        1 AS p_freq,
        d_date AS p_start_of_period
    FROM ce_warehouse.mv_date
    UNION ALL
    -- WEEKLY
    SELECT
        ce_warehouse.fx_ut_date_to_pdi(d_start_of_week, 2) AS pk_p,
        2 AS p_freq,
        d_start_of_week AS p_start_of_period
    FROM (
        SELECT DISTINCT d_start_of_week FROM ce_warehouse.mv_date
    ) d
    UNION ALL
    -- MONTHLY
    SELECT
        ce_warehouse.fx_ut_date_to_pdi(d_start_of_month, 3) AS pk_p,
        3 AS p_freq,
        d_start_of_month AS p_start_of_period
    FROM (
        SELECT DISTINCT d_start_of_month FROM ce_warehouse.mv_date
    ) d
    UNION ALL
    -- QUARTERLY
    SELECT
        ce_warehouse.fx_ut_date_to_pdi(d_start_of_quarter, 4) AS pk_p,
        4 AS p_freq,
        d_start_of_quarter AS p_start_of_period
    FROM (
        SELECT DISTINCT d_start_of_quarter FROM ce_warehouse.mv_date
    ) d
    UNION ALL
    -- YEARLY
    SELECT
        ce_warehouse.fx_ut_date_to_pdi(d_start_of_year, 5) AS pk_p,
        5 AS p_freq,
        d_start_of_year AS p_start_of_period
    FROM (
        SELECT DISTINCT d_start_of_year FROM ce_warehouse.mv_date
    ) d
),
_base AS (
    SELECT
