...
-- DROP TABLE IF EXISTS ce_warehouse.a_generic_audit;

CREATE TABLE IF NOT EXISTS ce_warehouse.a_generic_audit
(
...
CREATE INDEX IF NOT EXISTS a_generic_audit__table__idx
    ON ce_warehouse.a_generic_audit (table_name, table_pk, idx DESC);  -- index to optimize queries for specific tables/records, with most recent changes first

COMMENT ON TABLE ce_warehouse.a_generic_audit
