...
DECLARE
    ...
    _sid1 TEXT := (SELECT sid1 FROM ce_warehouse.c_series WHERE pk_series = COALESCE(NEW.fk_pk_series, OLD.fk_pk_series));
    _first_pdi INT;
    _last_pdi INT;
...
BEGIN
    IF TG_OP = 'INSERT' THEN
        ...
        INSERT INTO ce_warehouse.x_series_meta(fk_pk_series, ifreq, itype, sid1, first_pdi, last_pdi, has_values, new_values_utc)
        VALUES (
    ...
    ELSEIF TG_OP = 'DELETE' THEN
        ...
        VALUES (
            OLD.fk_pk_series, OLD.ifreq, OLD.itype, _first_pdi, _last_pdi, (_first_pdi IS NOT NULL)::BOOL, _now
        )
        ON CONFLICT (fk_pk_series, ifreq, itype)
        DO UPDATE
